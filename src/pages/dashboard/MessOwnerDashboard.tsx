  ...
  if (view === 'overview') {
    return (
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">{stats.map((s) => (<div key={s.label} className="card-interactive p-5 group"><div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${s.color} flex items-center justify-center text-2xl mb-3 group-hover:scale-110 group-hover:rotate-6 transition-transform duration-300`}>{s.icon}</div><div className="text-2xl font-bold text-slate-900 dark:text-white leading-tight">{s.value}</div><div className="text-xs text-slate-500 mt-0.5">{s.label}</div><div className="text-[11px] text-brand-500 mt-1">{s.sub}</div></div>))}</div>
